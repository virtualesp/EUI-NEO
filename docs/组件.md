...
当前 input 支持 focus、点击定位光标、拖动选择、Shift+方向键选择、Ctrl+A/C/X/V/Z、Cmd+A/C/X/V/Z、Ctrl+Y、Ctrl+Shift+Z、Cmd+Shift+Z、Backspace、Delete、Enter、单行和多行模式，并会把系统 IME 候选框锚定到当前光标位置。光标、选择、删除、撤销、重做和横向滚动基于 `TextPrimitive::measureTextMetrics(...)` 的 shaped caret stops；这些 stops 使用 UTF-8 byte offset，并与实际渲染的 fallback、emoji 缩放和 glyph advance 保持一致，适配 emoji、CJK、阿拉伯连字和组合符号。默认字体族是跨平台 `monospace`，默认占位文本是 `Hello EUI-NEO 😉`。暂时没有 IME 组合态显示。
...
- 容器 transform 会继承到组件内部子树渲染，包含 2D transform 和 2.5D 透视投影；transform 后的 hit-test 仍按布局矩形计算。
